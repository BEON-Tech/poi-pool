...
describe('Token contract', function () {
  ...
  beforeEach(async function () {
    ...
    hardhatContract = await upgrades.deployProxy(Contract, [], {
  ...
  describe('Certifiers', function () {
    ...
    it('Should fail when trying to register multiple certifiers with an invalid number of arguments', async function () {
      ...
      await expect(hardhatContract.addCertifiers(firstNames, lastNames, certifierIds, addresses)).to.be.revertedWith('Invalid arrays length');
    });
    ...
    it("Should fail if sender tries to register same certifier's wallet twice (or more)", async function () {
      ...
      await hardhatContract.addCertifier(firstName, lastName, certifierId, address1.address);
      await expect(hardhatContract.addCertifier(firstName, lastName, certifierId, address1.address)).to.be.revertedWith(
    ...
    it('Should allow only the governor to register certifiers', async function () {
      await expect(
        hardhatContract.connect(address1).addCertifier('TestFirstname2', 'TestLastname2', generateRandomId(), address2.address)
    ...
    it("Should fail if sender tries to get or verify an unregistered certifier's wallet", async function () {
      ...
      await expect(hardhatContract.getCertifier(address2.address)).to.be.revertedWith('Invalid wallet address');
    });
  ...
  describe('Applicants', function () {
    ...
    it('Should fail when trying to register multiple applicants with an invalid number of arguments', async function () {
      ...
      await expect(hardhatContract.addApprovedApplicants(firstNames, lastNames, applicantIds, addresses)).to.be.revertedWith(
    ...
    it("Should fail if sender tries to register same applicant's wallet twice (or more)", async function () {
      ...
      await hardhatContract.addApprovedApplicant(firstName, lastName, applicantId, address1.address);
      await expect(hardhatContract.addApprovedApplicant(firstName, lastName, applicantId, address1.address)).to.be.revertedWith(
        'Wallet address already in use'
      );
    });
    ...
    it('Should allow only the governor to register applicants', async function () {
      await expect(
        hardhatContract.connect(address1).addApprovedApplicant('TestFirstname2', 'TestLastname2', generateRandomId(), address2.address)
      ).to.be.revertedWith('The caller is not the governor.');
    });
    ...
    it("Should fail if sender tries to get or verify an unregistered applicant's wallet", async function () {
      ...
      await expect(hardhatContract.getApprovedApplicant(address2.address)).to.be.revertedWith('Invalid wallet address');
    });
  ...
  describe('Granted Applications', function () {
    ...
    it('Should fail while trying to register a granted application using invalid addresses', async function () {
      ...
      await expect(hardhatContract.addGrantedApplication(address3.address, address2.address, generateRandomId())).to.be.revertedWith(
        'Invalid certifier wallet address'
      );

      await expect(hardhatContract.addGrantedApplication(address1.address, address4.address, generateRandomId())).to.be.revertedWith(
    ...
    it('Should fail while trying to register a granted application with the same application ID twice', async function () {
      ...
      await expect(hardhatContract.addGrantedApplication(address1.address, address2.address, applicationId)).to.be.revertedWith(
