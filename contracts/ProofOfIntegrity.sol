...
contract ProofOfIntegrity is Initializable {
  ...
  address[] private _certifiersAccounts;
  address[] private _approvedApplicantsAccounts;
  uint256[] private _grantedApplicationIds;
   ...
   */
  function addCertifiers(
    string[] calldata _firstnames,
    string[] calldata _lastnames,
    uint256[] calldata _databaseIds,
    address[] calldata _wallets
  ) external onlyByGovernor {
    require(
      _firstnames.length == _lastnames.length && _firstnames.length == _databaseIds.length && _firstnames.length == _wallets.length,
  ...
  function _addCertifier(
    string calldata _firstname,
    string calldata _lastname,
    uint256 _databaseId,
    address _wallet
  ) internal onlyByGovernor {
    ...
    _certifiersAccounts.push(_wallet);
  }
   ...
   */
  function getCertifier(address _wallet)
    external
    view
    returns (
      uint256 _certifierId,
   ...
   */
  function addApprovedApplicants(
    string[] calldata _firstnames,
    string[] calldata _lastnames,
    uint256[] calldata _databaseIds,
    address[] calldata _wallets
  ...
  function _addApprovedApplicant(
    string calldata _firstname,
    string calldata _lastname,
    uint256 _databaseId,
    address _wallet
    ...
    _approvedApplicantsAccounts.push(_wallet);
  }
  ...
  function getApprovedApplicantsAccounts() external view returns (address[] memory) {
    return _approvedApplicantsAccounts;
  }
   ...
   */
  function getApprovedApplicant(address _wallet)
    external
    view
    returns (
   ...
   * @param _databaseId Record id for the applicant in the database.
   */
  function verifyApprovedApplicant(
    address _wallet,
    string calldata _firstname,
    string calldata _lastname,
    uint256 _databaseId
   ...
   */
  function addGrantedApplication(
    address _certifierWallet,
   ...
   */
  function addGrantedApplications(
    address[] calldata _certifierWallets,
    address[] calldata _applicantWallets,
    uint256[] calldata _databaseIds
  ) external onlyByGovernor {
    ...
    for (uint256 i = 0; i < _certifierWallets.length; i++) {
  ...
  function _addGrantedApplication(
    address _certifierWallet,
    address _applicantWallet,
    uint256 _databaseId
  ) internal onlyByGovernor {
    ...
    application.evidenceHash = keccak256(
      abi.encodePacked(certifiers[_certifierWallet].evidenceHash, approvedApplicants[_applicantWallet].evidenceHash)
    );
    _grantedApplicationIds.push(_databaseId);
   ...
   */
  function getGrantedApplication(uint256 _databaseId)
    external
    view
    returns (
      address _certifier,
